...
mix = interleave_datasets(
    [stream(f"{ER}/data_train/cpt/jsonl/hy_clean.jsonl"),
     stream(f"{ER}/data_train/cpt/jsonl/stem_hy_full.jsonl"),
     stream(f"{ER}/data_train/cpt/jsonl/stem_en_full.jsonl"),
...
args = TrainingArguments(
    ...
    learning_rate=float(os.environ.get("CPT_LR", "1e-4")),
...
trainer.train(resume_from_checkpoint=any(
    d.startswith("checkpoint") for d in (os.listdir(OUT) if os.path.isdir(OUT) else [])))
